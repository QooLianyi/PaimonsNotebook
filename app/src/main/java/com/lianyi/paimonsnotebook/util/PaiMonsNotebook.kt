...
val navigationBarHeight: Int
        by lazy {
            val resourceId = PaiMonsNoteBook.context.resources.getIdentifier(
            ...
            if (resourceId > 0) {
                //判断是否显示了导航栏
                val configShowNav = PaiMonsNoteBook.context.resources.getIdentifier(
...
fun openAndCloseAnimationHor(
    target: View, start: Int, end: Int, time: Long, block: () -> Unit = object : () -> Unit {
        override fun invoke() {
        }
    }
) {
...
fun showConfirmAlertDialog(
    ...
    title: String = "提示",
    content: String = "你确定吗",
    block: (isConfirm: Boolean) -> Unit
) {
    val layout =
        PopConfirmBinding.bind(LayoutInflater.from(context).inflate(R.layout.pop_confirm, null))
...
fun showSuccessInformationAlertDialog(context: Context, title: String) {
    val layout =
        PopSuccessBinding.bind(LayoutInflater.from(context).inflate(R.layout.pop_success, null))
...
fun showFailureAlertDialog(
    context: Context,
    title: String = "失败",
    message: String = ""
): AlertDialog {
    val layout =
        PopFailureBinding.bind(LayoutInflater.from(context).inflate(R.layout.pop_failure, null))
    val win = AlertDialog.Builder(context).setView(layout.root).create()
